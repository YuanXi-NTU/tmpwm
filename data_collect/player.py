# currently, use this code to replace respective file in rl_games to collect samples
import time
import gym
import numpy as np
import torch
...
class BasePlayer(object):
    def __init__(self, config):
        ...
        # self.buffer=VectorizedReplayBuffer([4096,self.obs_shape[0]],[4096,self.action_space.shape[0]],2000,self.device)###
        self.buffer_idx=0
        self.buffer_obs=torch.empty(1000,4096,self.observation_space.shape[0])
        self.buffer_next_obs=torch.empty(1000,4096,self.observation_space.shape[0])
        self.buffer_action=torch.empty(1000, 4096, self.action_space.shape[0])
        self.buffer_done=torch.empty(1000,4096)
        self.buffer_reward=torch.empty(1000,4096)
    ...
    def run(self):
        #print(self.is_rnn,self.games_num,self.n_game_life) false,2000,1 
        ...
        for _ in range(n_games):
            ...
            for n in range(self.max_steps):
                ...
                self.buffer_done[self.buffer_idx,:]=done
                self.buffer_reward[self.buffer_idx,:]=r
                self.buffer_obs[self.buffer_idx]=obses
                self.buffer_next_obs[self.buffer_idx]=self.obs
        ...
        print(sum_rewards)
        if sum_steps / games_played * n_game_life>500:
            import pickle
            buffer={'obs':self.buffer_obs.cpu(),'next_obs':self.buffer_next_obs.cpu(),
                    'action':self.buffer_action.cpu(),'reward':self.buffer_reward.cpu(),'done':self.buffer_done.cpu()}
            tmp=open('res_buffer.pickle','wb')
            pickle.dump(buffer,tmp)
            tmp.close()

...
class A2CBase:
    ...
    def init_rnn_step(self, batch_size, mb_rnn_states):
        ...
        steps_mask = torch.arange(0, batch_size * self.horizon_length, self.horizon_length, dtype=torch.long,
                                  device=self.ppo_device)
        play_mask = torch.arange(0, batch_size, 1, dtype=torch.long, device=self.ppo_device)
        steps_state = torch.arange(0, batch_size * self.horizon_length // self.seq_len,
                                   self.horizon_length // self.seq_len, dtype=torch.long, device=self.ppo_device)
    ...
    def play_steps(self):
        ...
        #    print('mean_rew:',torch.sum(mb_rewards,(0,2)))
...
class ContinuousA2CBase(A2CBase):
    def __init__(self, base_name, config):
        ...
        player_config['name']='Ant'
        player_config['full_experiment_name']='Ant'

        self.player = PpoPlayerContinuous(player_config)
        from isaacgym import gymapi
        from isaacgymenvs.ant import Ant
        import yaml
        y1 = yaml.load(open('/home/yuanxi20/isaacgym/IsaacGymEnvs/isaacgymenvs/cfg/config.yaml'), yaml.FullLoader)
        y1['task_name']='Ant'
        y2 = yaml.load(open('/home/yuanxi20/isaacgym/IsaacGymEnvs/isaacgymenvs/cfg/task/Ant-test.yaml'),
                       yaml.FullLoader)
        y1.update(y2)
        self.player.env=Ant(y1,'cuda:0',-1,True)
    ...
    def train(self):
        ...
        while True:
            epoch_num = self.update_epoch()
            step_time, play_time, update_time, \
            sum_time, a_losses, c_losses, b_losses, entropies, kls, last_lr, lr_mul = self.train_epoch()
            # import ipdb
            # ipdb.set_trace()
            
            # if epoch_num%test_epoch_num==0:
                ...
                if epoch_num > self.max_epochs:
                    self.save(os.path.join(self.nn_dir,
                                           'last_' + self.config['name'] + 'ep' + str(epoch_num) + 'rew' + str(
                                               mean_rewards)))
                    import pickle
                    tmp = open('./res_exp.pickle', 'wb')
                    pickle.dump(self.experience_buffer, tmp)
                    tmp.close()
                    print('MAX EPOCHS NUM!')
                    should_exit = True
